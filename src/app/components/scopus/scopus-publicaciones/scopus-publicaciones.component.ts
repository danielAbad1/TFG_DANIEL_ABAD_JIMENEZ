  ...
  implements OnInit
{
  ...
  async loadPublicacionesPolitecnica(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `🔢 Total publicaciones encontradas: ${this.totalResultadosEsperados}`
      );
      ...
      console.error('❌ Error al cargar publicaciones SPARQL:', err);
    } finally {
      this.cargando = false;
    }
  }
  ...
  async aplicarFiltroPorKeyword(): Promise<void> {
    ...
    try {
      ...
      if (err.status === 400) {
        ...
        console.error('❌ Error al filtrar por keywords Scopus:', err);
      }
      this.scopusMatchedEids = [];
      this.scopusMatchedTitles = [];
      this.aplicarFiltro();
